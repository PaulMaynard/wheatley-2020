...
export class LevelScreen extends Screen {
    ...
    render(display) {
        ...
        // render player stats
        display.drawText(dim.x - 19, 1, 'Grade: ' + grade(this.player.health, this.player.props.maxhealth ?? 1) + ' (' +
        ...
        if (this.player.weapon) {
            ...
            display.drawText(dim.x - 17, 4, w.name + ' (' +
                (w.props.attack?.[0] ?? 'N/A') + ' ' +
                (w.props.attack?.[2] ?? 'N/A') + ')');
        }
        else {
            display.drawText(dim.x - 17, 4, 'bare hands (' + ')');
        }
        let i = 6;
